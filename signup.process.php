...
if (!empty($data_unique['email_user'])) {
    echo "<script>alert('Email already in used');location.href='signup.php';</script>";
} else {
    if (empty($nama) || empty($jenis_kelamin) || empty($email) || empty($password) || empty($alamat)) {
        echo "<script>alert('Data tidak boleh kosong');location.href='signup.php';</script>";
    } else {
        ...
        if (!empty($_FILES["foto"])) {
            if ($_FILES["foto"]["size"] < (5 * 1024 * 1024)) {
                ...
                echo "<script>alert('Daftar berhasil, gagal memasukkan foto (Maks 5MB)');location.href='signup.php';</script>";
            }
        }
        if ($daftar) {
            echo "<script>alert('Daftar berhasil');location.href='login.php';</script>";
        } else {
            echo "<script>alert('Daftar gagal');location.href='signup.php';</script>";
        }
    }
}
?>