...
if ($_SESSION['status_login'] == false) {
    echo "<script>alert('Harap login sebelum melakukan pembelian');location.href='login.html';</script>";
} elseif ($_POST['quantity'] < 1) {
    echo "<script>alert('Minimum pembelian 1 barang');location.href='transaksi.php?id_merch=" . $_POST['id_merch'] . "';</script>";
} else {
